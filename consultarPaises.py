    ...
    try:
        ...
        with open(iniciar['entrada'],"r") as archivo:
            for linea in archivo:
                cuitTitula = linea
        resultado = iniciar['cliente'].service.consultarPaises(authRequest={'token': iniciar['token'], 'sign': iniciar['sign'], 'cuitRepresentada': iniciar['cuitRepresentada']})
        print(resultado)
        resultado = resultado[0]
        archivo=open (iniciar['salida'],'w')
        for a in resultado:
            for b in a[1] :
                archivo.write(str(b.codigo)+'\t'+str(b.cuit)+'\t'+b.nombre+'\t'+b.tipoSujeto+'\n')
        archivo.close()
